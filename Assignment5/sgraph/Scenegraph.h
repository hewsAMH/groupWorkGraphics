...
namespace sgraph
{
  ...
  public:
    ...
    /**
     * Get all lights in the scene graph and transform them to view space
  ...
  private:
     ...
     * @param modelMatrix accumulated model transformation up to this node
     * @param viewMatrix the view matrix
    ...
    void collectLightsFromNode(SGNode *node, const glm::mat4 &modelMatrix, const glm::mat4 &viewMatrix, vector<util::Light> &lights)
    {
      ...
      if (leafNode)
      {
        ...
        if (glm::length(nodeLight.getAmbient()) > 0 ||
            glm::length(nodeLight.getDiffuse()) > 0 ||
            glm::length(nodeLight.getSpecular()) > 0)
        {

          // Transform the light to view space
          util::Light transformedLight = util::transformLight(nodeLight, viewMatrix * modelMatrix);
